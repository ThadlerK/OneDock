...
def convert_xtc_to_pdb(bioemu_workdir):
    """
    Converts BioEmu samples.xtc + topology.pdb to PDB files (one per frame).
    """
    topology_path = os.path.join(bioemu_workdir, "ensembles", "topology.pdb")
    xtc_path      = os.path.join(bioemu_workdir, "ensembles", "samples.xtc")
    pdb_out_dir   = os.path.join(bioemu_workdir, "pdb_backbone")

    print(f"Converting XTC to PDB in {pdb_out_dir}...")
    ...
    if not os.path.exists(xtc_path):
        print(f"Error: XTC file not found at {xtc_path}")
        return []
    ...
    for i, ts in enumerate(u.trajectory, start=1):
        out_path = os.path.join(pdb_out_dir, f"sample_{i:06d}.pdb")
        u.atoms.write(out_path)   # writes atoms from topology with coords from this frame
...
if __name__ == "__main__":
    ...
    # 2. Convert to PDBs
    all_pdbs = convert_xtc_to_pdb(args.working_dir)
