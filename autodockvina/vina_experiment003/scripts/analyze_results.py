...
# 1. DATEN LADEN
file_path = "/workspace/vina_experiment002/outputs/summary_scores copy.csv" 
try:
...
df['Unique_Ligand'] = df['Ligand_File'] + "_" + df['Ligand_Index'].astype(str)
...
sns.boxplot(data=df, x="Ligand_File", y="Affinity_kcal_mol", hue="Receptor", palette="muted")
...
top_hits_per_lib = df_sorted.groupby("Ligand_File").head(top_n)

# FacetGrid: Erstellt für jede Library einen eigenen kleinen Plot
g = sns.FacetGrid(top_hits_per_lib, col="Ligand_File", col_wrap=3, height=4, sharey=False, sharex=False)
g.map_dataframe(sns.barplot, x="Affinity_kcal_mol", y="Unique_Ligand", hue="Receptor", dodge=False, palette="viridis")
...
print(df_sorted[['Receptor', 'Unique_Ligand', 'Affinity_kcal_mol']].head(10).to_string(index=False))